...
def previous_task(m, i, j, k):
    if('previous' in tasks[(i, j)] and k in tasks[(i, j)]['machine']):
        return m.start[(i, j), k] >= m.start[(i, j-1), tasks[(i, j-1)]['machine']] + tasks[(i, j-1)]['time']
    else:
        return Constraint.Skip
...
def const_1(m, i1, j1, i2, j2, k):
    if(i1 < i2 and k in tasks[(i1, j1)]['machine'] and k in tasks[(i2, j2)]['machine']):
        return m.start[(i1, j1, k)] + tasks[(i1, j1)]['time'] <= m.start[(i2, j2, k)] + horizon*(1-m.binary[(i1, j1), (i2, j2), k])
    else:
        return Constraint.Skip
...
def const_2(m, i1, j1, i2, j2, k):
    if(i1 < i2 and k in tasks[(i1, j1)]['machine'] and k in tasks[(i2, j2)]['machine']):
        return m.start[(i2, j2, k)] + tasks[(i2, j2)]['time'] <= m.start[(i1, j1, k)] + horizon*(m.binary[(i1, j1), (i2, j2), k])
    else:
        return Constraint.Skip
...
results = opt.solve(instance, symbolic_solver_labels=True, tee=True, load_solutions=True)
...
plot(fig, filename='job_shop', image="png", image_filename="job_shop", auto_open=True)
